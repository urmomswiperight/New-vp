...
@D:/New-vp-main/get-shit-done/references/ui-brand.md
@D:/New-vp-main/get-shit-done/references/revision-loop.md
@D:/New-vp-main/get-shit-done/references/gate-prompts.md
@D:/New-vp-main/get-shit-done/references/agent-contracts.md
@D:/New-vp-main/get-shit-done/references/gates.md
</required_reading>
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@D:/New-vp-main/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
Set `TEXT_MODE=true` if `--text` is present in $ARGUMENTS OR `text_mode` from init JSON is `true`. When `TEXT_MODE` is active, replace every `AskUserQuestion` call with a plain-text numbered list and ask the user to type their choice number. This is required for Cline remote sessions (`/rc` mode) where TUI menus don't work through the Claude App.
...
**Project instructions:** Read .clinerules if exists — follow project-specific guidelines
**Project skills:** Check .cline/skills/ or .agents/skills/ directory (if either exists) — read SKILL.md files, research should account for project skill patterns
</additional_context>
...
**If found:**
1. Read template: `D:/New-vp-main/get-shit-done/templates/VALIDATION.md`
...
SPIKE_FINDINGS_PATH=$(ls ./.cline/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1)
SKETCH_FINDINGS_PATH=$(ls ./.cline/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1)
```
...
**Project instructions:** Read .clinerules if exists — follow project-specific guidelines
**Project skills:** Check .cline/skills/ or .agents/skills/ directory (if either exists) — read SKILL.md files, plans should account for project skill rules

${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @D:/New-vp-main/get-shit-done/references/tdd.md to all eligible tasks:
...
**Project instructions:** Read .clinerules if exists — verify plans honor project guidelines
**Project skills:** Check .cline/skills/ or .agents/skills/ directory (if either exists) — verify plans account for project skill rules
</verification_context>
...
stdio deadlocks with MCP servers — see Cline issue anthropics/claude-code#28126):
...
3. **Clean up stale task directories:**
   ```powershell
   # Remove stale subagent task dirs (Cline never cleans these on crash)
   Remove-Item -Recurse -Force "$env:USERPROFILE\.claude\tasks\*" -ErrorAction SilentlyContinue
   ```
4. **Reduce MCP server count:** Temporarily disable non-essential MCP servers in settings.json
5. **Retry:** Restart Cline and run `/gsd-plan-phase` again
