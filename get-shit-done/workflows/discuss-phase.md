...
<required_reading>
@D:/New-vp-main/get-shit-done/references/domain-probes.md
@D:/New-vp-main/get-shit-done/references/gate-prompts.md
@D:/New-vp-main/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
This is required for Cline remote sessions (`/rc` mode) where the Claude App
...
- Read and execute @D:/New-vp-main/get-shit-done/workflows/discuss-phase-power.md end-to-end
...
SPIKE_FINDINGS=$(ls ./.cline/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1)
SKETCH_FINDINGS=$(ls ./.cline/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1)
...
```bash
PROFILE_CONTENT=$(cat "D:/New-vp-main/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @D:/New-vp-main/agents/gsd-advisor-researcher.md for your role and instructions.
...
- This is required for Cline remote sessions (`/rc` mode) where TUI menus
...
**Full step-by-step instructions:** @D:/New-vp-main/get-shit-done/workflows/discuss-phase-power.md
