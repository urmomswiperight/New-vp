...
async function main() {
  try {
    ...
    const emailRes = await client.query(`SELECT COUNT(*) FROM "Lead" WHERE "email" IS NOT NULL AND "email" != ''`);
    console.log('Total leads with email:', emailRes.rows[0].count);
    ...
    const readyEmailRes = await client.query(`SELECT COUNT(*) FROM "Lead" WHERE "status" = 'New' AND "email" IS NOT NULL AND "email" != ''`);
    console.log('New leads with email (Ready):', readyEmailRes.rows[0].count);
