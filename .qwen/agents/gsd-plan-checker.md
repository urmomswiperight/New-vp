...
@D:/New-vp-main/.qwen/get-shit-done/references/thinking-models-planning.md

For calibration on scoring and issue identification, reference these examples:
@D:/New-vp-main/.qwen/get-shit-done/references/few-shot-examples/plan-checker.md
...
## Dimension 10: QWEN.md Compliance

**Question:** Do plans respect project-specific conventions, constraints, and requirements from QWEN.md?

**Process:**
1. Read `./QWEN.md` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns QWEN.md explicitly forbids
5. Flag plans that skip steps QWEN.md explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern QWEN.md explicitly forbids
- Plan skips a required step (e.g., QWEN.md says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts QWEN.md conventions
- Plan creates files in locations that violate QWEN.md's architectural constraints
- Plan ignores security requirements documented in QWEN.md

**Skip condition:** If no `./QWEN.md` exists in the working directory, output: "Dimension 10: SKIPPED (no QWEN.md found)" and move on.
...
issue:
  ...
  severity: blocker
  description: "Plan uses Jest for testing but QWEN.md requires Vitest"
  plan: "01"
  task: 1
  claude_md_rule: "Testing: Always use Vitest, never Jest"
  plan_action: "Install Jest and create test suite..."
  fix_hint: "Replace Jest with Vitest per project QWEN.md"
```
...
issue:
  dimension: claude_md_compliance
  severity: warning
  description: "Plan does not include lint step required by QWEN.md"
  plan: "02"
...
- [ ] QWEN.md compliance checked (plans respect project conventions)
