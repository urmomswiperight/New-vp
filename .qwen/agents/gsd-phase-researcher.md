...
**QWEN.md enforcement:** If `./QWEN.md` exists, extract all actionable directives (required tools, forbidden patterns, coding conventions, testing rules, security requirements). Include a `## Project Constraints (from QWEN.md)` section in RESEARCH.md listing these directives so the planner can verify compliance. Treat QWEN.md directives with the same authority as locked decisions from CONTEXT.md — research should not recommend approaches that contradict them.
</project_context>
...
@D:/New-vp-main/.qwen/get-shit-done/references/thinking-models-research.md
...
```bash
node "D:/New-vp-main/.qwen/get-shit-done/bin/gsd-tools.cjs" graphify query "<capability-keyword>" --budget 1500
```
