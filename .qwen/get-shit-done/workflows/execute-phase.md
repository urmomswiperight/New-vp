...
- **Qwen Code:** Uses `Task(subagent_type="gsd-executor", ...)` — blocks until complete, returns result
...
@D:/New-vp-main/.qwen/get-shit-done/references/agent-contracts.md
@D:/New-vp-main/.qwen/get-shit-done/references/context-budget.md
@D:/New-vp-main/.qwen/get-shit-done/references/gates.md
</required_reading>
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @D:/New-vp-main/.qwen/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @D:/New-vp-main/.qwen/get-shit-done/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `D:/New-vp-main/.qwen/get-shit-done/workflows/execute-plan.md` **inline**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       <execution_context>
       @D:/New-vp-main/.qwen/get-shit-done/workflows/execute-plan.md
       @D:/New-vp-main/.qwen/get-shit-done/templates/summary.md
       @D:/New-vp-main/.qwen/get-shit-done/references/checkpoints.md
       @D:/New-vp-main/.qwen/get-shit-done/references/tdd.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@D:/New-vp-main/.qwen/get-shit-done/references/executor-examples.md'}
       </execution_context>
       ...
       ` : ''}
       - ./QWEN.md (Project instructions, if exists — follow project-specific guidelines and coding conventions)
...
Read and follow `D:/New-vp-main/.qwen/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
...
<failure_handling>
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Qwen Code bug, not GSD. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
