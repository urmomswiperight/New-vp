...
- **Global scope**: `D:/New-vp-main/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/New-vp-main/.opencode/notes/`)
...
2. Glob `D:/New-vp-main/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (D:/New-vp-main/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/New-vp-main/.opencode/notes/` — works in any directory
