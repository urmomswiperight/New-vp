...
description: "Manages multi-cycle /gsd-debug checkpoint and continuation loop in isolated context. Spawns gsd-debugger agents, handles checkpoints via conversational prompting, dispatches specialist skills, applies fixes. Returns compact summary to main context. Spawned by /gsd-debug command."
---
...
**Anti-heredoc rule:** never use `launch-process(cat << 'EOF')` or heredoc commands for file creation. Always use the Write tool.
