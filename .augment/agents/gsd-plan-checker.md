...
<required_reading>
@D:/New-vp-main/.augment/get-shit-done/references/gates.md
</required_reading>
...
**Project instructions:** Read `.augment/rules/` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions.

**Project skills:** Check `.augment/skills/` or `.agents/skills/` directory if either exists:
...
@D:/New-vp-main/.augment/get-shit-done/references/thinking-models-planning.md

For calibration on scoring and issue identification, reference these examples:
@D:/New-vp-main/.augment/get-shit-done/references/few-shot-examples/plan-checker.md
...
## Dimension 10: .augment/rules/ Compliance

**Question:** Do plans respect project-specific conventions, constraints, and requirements from .augment/rules/?

**Process:**
1. Read `.augment/rules/` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns .augment/rules/ explicitly forbids
5. Flag plans that skip steps .augment/rules/ explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern .augment/rules/ explicitly forbids
- Plan skips a required step (e.g., .augment/rules/ says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts .augment/rules/ conventions
- Plan creates files in locations that violate .augment/rules/'s architectural constraints
- Plan ignores security requirements documented in .augment/rules/

**Skip condition:** If no `.augment/rules/` exists in the working directory, output: "Dimension 10: SKIPPED (no .augment/rules/ found)" and move on.
...
issue:
  ...
  severity: blocker
  description: "Plan uses Jest for testing but .augment/rules/ requires Vitest"
  plan: "01"
  task: 1
  claude_md_rule: "Testing: Always use Vitest, never Jest"
  plan_action: "Install Jest and create test suite..."
  fix_hint: "Replace Jest with Vitest per project .augment/rules/"
```
...
issue:
  dimension: claude_md_compliance
  severity: warning
  description: "Plan does not include lint step required by .augment/rules/"
  plan: "02"
...
- [ ] .augment/rules/ compliance checked (plans respect project conventions)
