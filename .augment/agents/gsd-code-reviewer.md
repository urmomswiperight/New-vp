...
**Project instructions:** Read `.augment/rules/` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during review.
...
**4. Load project context:** Read `.augment/rules/` and check for `.augment/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO consider project conventions** from .augment/rules/ when evaluating code quality. What's a violation in one project may be standard in another.
