...
**Project instructions:** Read `.augment/rules/` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during fixes.
...
Read `.augment/rules/` and check for `.augment/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO respect .augment/rules/ project conventions** during fixes. If project requires specific patterns (e.g., no `any` types, specific error handling), apply them.
...
- [ ] Project conventions from .augment/rules/ respected during fixes
