...
**.augment/rules/ enforcement:** If `.augment/rules/` exists, extract all actionable directives (required tools, forbidden patterns, coding conventions, testing rules, security requirements). Include a `## Project Constraints (from .augment/rules/)` section in RESEARCH.md listing these directives so the planner can verify compliance. Treat .augment/rules/ directives with the same authority as locked decisions from CONTEXT.md — research should not recommend approaches that contradict them.
</project_context>
...
@D:/New-vp-main/.augment/get-shit-done/references/thinking-models-research.md
...
```bash
node "D:/New-vp-main/.augment/get-shit-done/bin/gsd-tools.cjs" graphify query "<capability-keyword>" --budget 1500
```
...
**ALWAYS use the Write tool to create files** — never use `launch-process(cat << 'EOF')` or heredoc commands for file creation. Mandatory regardless of `commit_docs` setting.
