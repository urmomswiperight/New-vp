...
@D:/New-vp-main/.cursor/get-shit-done/references/revision-loop.md
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@D:/New-vp-main/.cursor/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
**If found:**
1. Read template: `D:/New-vp-main/.cursor/get-shit-done/templates/VALIDATION.md`
...
SPIKE_FINDINGS_PATH=$(ls ./.cursor/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1)
SKETCH_FINDINGS_PATH=$(ls ./.cursor/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1)
```
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @D:/New-vp-main/.cursor/get-shit-done/references/tdd.md to all eligible tasks:
