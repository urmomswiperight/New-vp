...
@D:/New-vp-main/.trae/get-shit-done/references/thinking-models-planning.md

For calibration on scoring and issue identification, reference these examples:
@D:/New-vp-main/.trae/get-shit-done/references/few-shot-examples/plan-checker.md
...
## Dimension 10: .trae/rules/ Compliance

**Question:** Do plans respect project-specific conventions, constraints, and requirements from .trae/rules/?

**Process:**
1. Read `.trae/rules/` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns .trae/rules/ explicitly forbids
5. Flag plans that skip steps .trae/rules/ explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern .trae/rules/ explicitly forbids
- Plan skips a required step (e.g., .trae/rules/ says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts .trae/rules/ conventions
- Plan creates files in locations that violate .trae/rules/'s architectural constraints
- Plan ignores security requirements documented in .trae/rules/

**Skip condition:** If no `.trae/rules/` exists in the working directory, output: "Dimension 10: SKIPPED (no .trae/rules/ found)" and move on.
...
issue:
  ...
  severity: blocker
  description: "Plan uses Jest for testing but .trae/rules/ requires Vitest"
  plan: "01"
  task: 1
  claude_md_rule: "Testing: Always use Vitest, never Jest"
  plan_action: "Install Jest and create test suite..."
  fix_hint: "Replace Jest with Vitest per project .trae/rules/"
```
...
issue:
  dimension: claude_md_compliance
  severity: warning
  description: "Plan does not include lint step required by .trae/rules/"
  plan: "02"
...
- [ ] .trae/rules/ compliance checked (plans respect project conventions)
