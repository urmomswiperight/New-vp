...
```bash
ls -d .kilo 2>/dev/null && echo "kilo" || (ls -d .codebuddy/get-shit-done 2>/dev/null && echo "claude") || echo "unknown"
```
...
counts by running Glob on the layout-resolved canonical locations above, not from memory or CODEBUDDY.md.
