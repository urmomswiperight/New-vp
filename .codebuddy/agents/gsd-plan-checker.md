...
<required_reading>
@D:/New-vp-main/.codebuddy/get-shit-done/references/gates.md
</required_reading>
...
**Project instructions:** Read `CODEBUDDY.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions.

**Project skills:** Check `.codebuddy/skills/` or `.agents/skills/` directory if either exists:
...
@D:/New-vp-main/.codebuddy/get-shit-done/references/thinking-models-planning.md

For calibration on scoring and issue identification, reference these examples:
@D:/New-vp-main/.codebuddy/get-shit-done/references/few-shot-examples/plan-checker.md
...
## Dimension 10: CODEBUDDY.md Compliance

**Question:** Do plans respect project-specific conventions, constraints, and requirements from CODEBUDDY.md?

**Process:**
1. Read `CODEBUDDY.md` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns CODEBUDDY.md explicitly forbids
5. Flag plans that skip steps CODEBUDDY.md explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern CODEBUDDY.md explicitly forbids
- Plan skips a required step (e.g., CODEBUDDY.md says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts CODEBUDDY.md conventions
- Plan creates files in locations that violate CODEBUDDY.md's architectural constraints
- Plan ignores security requirements documented in CODEBUDDY.md

**Skip condition:** If no `CODEBUDDY.md` exists in the working directory, output: "Dimension 10: SKIPPED (no CODEBUDDY.md found)" and move on.
...
issue:
  ...
  severity: blocker
  description: "Plan uses Jest for testing but CODEBUDDY.md requires Vitest"
  plan: "01"
  task: 1
  claude_md_rule: "Testing: Always use Vitest, never Jest"
  plan_action: "Install Jest and create test suite..."
  fix_hint: "Replace Jest with Vitest per project CODEBUDDY.md"
```
...
issue:
  dimension: claude_md_compliance
  severity: warning
  description: "Plan does not include lint step required by CODEBUDDY.md"
  plan: "02"
...
- [ ] CODEBUDDY.md compliance checked (plans respect project conventions)
