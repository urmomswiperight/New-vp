...
**Project instructions:** Read `CODEBUDDY.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during review.
...
**4. Load project context:** Read `CODEBUDDY.md` and check for `.codebuddy/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO consider project conventions** from CODEBUDDY.md when evaluating code quality. What's a violation in one project may be standard in another.
