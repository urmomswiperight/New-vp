...
**Project instructions:** Read `CODEBUDDY.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during fixes.
...
Read `CODEBUDDY.md` and check for `.codebuddy/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO respect CODEBUDDY.md project conventions** during fixes. If project requires specific patterns (e.g., no `any` types, specific error handling), apply them.
...
- [ ] Project conventions from CODEBUDDY.md respected during fixes
