...
**Project instructions:** Read `.windsurf/rules` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during fixes.
...
Read `.windsurf/rules` and check for `.windsurf/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO respect .windsurf/rules project conventions** during fixes. If project requires specific patterns (e.g., no `any` types, specific error handling), apply them.
...
- [ ] Project conventions from .windsurf/rules respected during fixes
