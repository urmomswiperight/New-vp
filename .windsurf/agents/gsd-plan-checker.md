...
**Project instructions:** Read `.windsurf/rules` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions.

**Project skills:** Check `.windsurf/skills/` or `.agents/skills/` directory if either exists:
...
@D:/New-vp-main/.windsurf/get-shit-done/references/thinking-models-planning.md

For calibration on scoring and issue identification, reference these examples:
@D:/New-vp-main/.windsurf/get-shit-done/references/few-shot-examples/plan-checker.md
...
## Dimension 10: .windsurf/rules Compliance

**Question:** Do plans respect project-specific conventions, constraints, and requirements from .windsurf/rules?

**Process:**
1. Read `.windsurf/rules` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns .windsurf/rules explicitly forbids
5. Flag plans that skip steps .windsurf/rules explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern .windsurf/rules explicitly forbids
- Plan skips a required step (e.g., .windsurf/rules says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts .windsurf/rules conventions
- Plan creates files in locations that violate .windsurf/rules's architectural constraints
- Plan ignores security requirements documented in .windsurf/rules

**Skip condition:** If no `.windsurf/rules` exists in the working directory, output: "Dimension 10: SKIPPED (no .windsurf/rules found)" and move on.
...
issue:
  ...
  severity: blocker
  description: "Plan uses Jest for testing but .windsurf/rules requires Vitest"
  plan: "01"
  task: 1
  claude_md_rule: "Testing: Always use Vitest, never Jest"
  plan_action: "Install Jest and create test suite..."
  fix_hint: "Replace Jest with Vitest per project .windsurf/rules"
```
...
issue:
  dimension: claude_md_compliance
  severity: warning
  description: "Plan does not include lint step required by .windsurf/rules"
  plan: "02"
...
- [ ] .windsurf/rules compliance checked (plans respect project conventions)
