...
**Project instructions:** Read `.windsurf/rules` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during review.
...
**4. Load project context:** Read `.windsurf/rules` and check for `.windsurf/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO consider project conventions** from .windsurf/rules when evaluating code quality. What's a violation in one project may be standard in another.
