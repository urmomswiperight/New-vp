...
function buildAgentSkillsBlock(config, agentType, projectRoot) {
  ...
  for (const skillPath of skillPaths) {
    ...
    // Support global: prefix for skills installed at ~/.windsurf/skills/ (#1992)
    if (skillPath.startsWith('global:')) {
      ...
      if (!fs.existsSync(globalSkillMd)) {
        process.stderr.write(`[agent-skills] WARNING: Global skill not found at "~/.windsurf/skills/${skillName}/SKILL.md" — skipping\n`);
        continue;
      }
      ...
      validPaths.push({ ref: `${globalSkillDir}/SKILL.md`, display: `~/.windsurf/skills/${skillName}` });
      continue;
    }
