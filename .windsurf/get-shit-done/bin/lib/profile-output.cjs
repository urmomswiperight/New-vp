 ...
 *   - generate-claude-profile: Developer Profile section in .windsurf/rules
 *   - generate-claude-md: full .windsurf/rules with managed sections
 */
...
const CLAUDE_MD_FALLBACKS = {
  ...
  skills: 'No project skills found. Add skills to any of: `.windsurf/skills/`, `.agents/skills/`, `.cursor/skills/`, `.github/skills/`, or `.codex/skills/` with a `SKILL.md` index file.',
};
...
// ─── .windsurf/rules Section Generators ─────────────────────────────────────────────
 ...
 * (name + description) for each. Returns a table summary for .windsurf/rules so
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  let targetPath;
  if (options.global) {
    targetPath = path.join(os.homedir(), '.claude', '.windsurf/rules');
  } else if (options.output) {
    targetPath = path.isAbsolute(options.output) ? options.output : path.join(cwd, options.output);
  } else {
    // Read claude_md_path from config, default to ./.windsurf/rules
    let configClaudeMdPath = './.windsurf/rules';
    try {
