...
<required_reading>
@D:/New-vp-main/.windsurf/get-shit-done/references/domain-probes.md
@D:/New-vp-main/.windsurf/get-shit-done/references/gate-prompts.md
@D:/New-vp-main/.windsurf/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
Do NOT retry the conversational prompting or generate more questions when "Other" is selected with empty text.
...
This is required for Windsurf remote sessions (`/rc` mode) where the Claude App
...
**If `response_language` is set:** All user-facing questions, prompts, and explanations in this workflow MUST be presented in `{response_language}`. This includes conversational prompting labels, option text, gray area descriptions, and discussion summaries. Technical terms, code, and file paths remain in English. Subagent prompts stay in English — only user-facing output is translated.
...
- Read and execute @D:/New-vp-main/.windsurf/get-shit-done/workflows/discuss-phase-power.md end-to-end
- Do not continue with the steps below

**All mode** — If `--all` is present in ARGUMENTS:
- In `present_gray_areas`: auto-select ALL gray areas without asking the user (skips the conversational prompting selection step)
...
SPIKE_FINDINGS=$(ls ./.windsurf/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1)
SKETCH_FINDINGS=$(ls ./.windsurf/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1)
...
```bash
PROFILE_CONTENT=$(cat "D:/New-vp-main/.windsurf/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
**If `--auto` or `--all`:** Auto-select ALL gray areas. Log: `[--auto/--all] Selected all gray areas: [list area names].` Skip the conversational prompting below and continue directly to discuss_areas with all areas selected.
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @D:/New-vp-main/.windsurf/agents/gsd-advisor-researcher.md for your role and instructions.
...
- This is required for Windsurf remote sessions (`/rc` mode) where TUI menus
...
**Full step-by-step instructions:** @D:/New-vp-main/.windsurf/get-shit-done/workflows/discuss-phase-power.md
