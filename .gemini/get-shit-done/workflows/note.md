...
- **Global scope**: `D:/New-vp-main/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/New-vp-main/.gemini/notes/`)
...
2. Glob `D:/New-vp-main/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (D:/New-vp-main/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/New-vp-main/.gemini/notes/` — works in any directory
