...
**.clinerules enforcement:** If `.clinerules` exists, extract all actionable directives (required tools, forbidden patterns, coding conventions, testing rules, security requirements). Include a `## Project Constraints (from .clinerules)` section in RESEARCH.md listing these directives so the planner can verify compliance. Treat .clinerules directives with the same authority as locked decisions from CONTEXT.md — research should not recommend approaches that contradict them.
</project_context>
...
@D:/New-vp-main/get-shit-done/references/thinking-models-research.md
...
```bash
node "D:/New-vp-main/get-shit-done/bin/gsd-tools.cjs" graphify query "<capability-keyword>" --budget 1500
```
