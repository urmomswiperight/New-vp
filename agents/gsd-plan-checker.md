...
**Project instructions:** Read `.clinerules` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions.

**Project skills:** Check `.cline/skills/` or `.agents/skills/` directory if either exists:
...
@D:/New-vp-main/get-shit-done/references/thinking-models-planning.md

For calibration on scoring and issue identification, reference these examples:
@D:/New-vp-main/get-shit-done/references/few-shot-examples/plan-checker.md
...
## Dimension 10: .clinerules Compliance

**Question:** Do plans respect project-specific conventions, constraints, and requirements from .clinerules?

**Process:**
1. Read `.clinerules` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns .clinerules explicitly forbids
5. Flag plans that skip steps .clinerules explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern .clinerules explicitly forbids
- Plan skips a required step (e.g., .clinerules says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts .clinerules conventions
- Plan creates files in locations that violate .clinerules's architectural constraints
- Plan ignores security requirements documented in .clinerules

**Skip condition:** If no `.clinerules` exists in the working directory, output: "Dimension 10: SKIPPED (no .clinerules found)" and move on.
...
issue:
  ...
  severity: blocker
  description: "Plan uses Jest for testing but .clinerules requires Vitest"
  plan: "01"
  task: 1
  claude_md_rule: "Testing: Always use Vitest, never Jest"
  plan_action: "Install Jest and create test suite..."
  fix_hint: "Replace Jest with Vitest per project .clinerules"
```
...
issue:
  dimension: claude_md_compliance
  severity: warning
  description: "Plan does not include lint step required by .clinerules"
  plan: "02"
...
- [ ] .clinerules compliance checked (plans respect project conventions)
